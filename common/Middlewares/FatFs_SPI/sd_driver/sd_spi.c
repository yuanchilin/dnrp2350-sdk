...
bool sd_spi_transfer(sd_card_t *pSD, const uint8_t *tx, uint8_t *rx,
                     size_t length) {
    /* 使用轮询 SPI，避免 DMA 信号量死锁 */
    if (tx && rx) {
        spi_write_read_blocking(pSD->spi->hw_inst, tx, rx, length);
    } else if (tx) {
        spi_write_blocking(pSD->spi->hw_inst, tx, length);
    } else {
        spi_read_blocking(pSD->spi->hw_inst, SPI_FILL_CHAR, rx, length);
    }
    return true;
}
...
uint8_t sd_spi_write(sd_card_t *pSD, const uint8_t value) {
    ...
    spi_write_read_blocking(pSD->spi->hw_inst, &value, &received, 1);
    return received;
}
